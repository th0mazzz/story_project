...
def discover():
    ...
    def getOriginal(storyname):
        return story.getAuthor(storyname)
    
    def getPrevious(storyname, content):
        return story.getSpecificAuthor(storyname, content)
    
    storiesList = story.getDiscoverDict(session['user']) #list of all stories
    ...
    return render_template('discover.html', keys = storiesList.keys(), dct = storiesList, ori = getOriginal, prev = getPrevious)
...
def edit():
    ...
    #check to see which story you are editing
    try:
        storyname = request.args['story']
    except:
        return redirect("/discover")
    
    #get the content of last edit
    if 'user' in session:
...
def editChanges():
    ...
    #get title and content of edit and add it to database
    storyname = request.args['story']
...
def contribute():
    ...
    '''This function redirects the user to the login page if they aren't logged in.
       otherwise, it checks if you already submitted a contribution and posted something. 
       If you posted, it will add the change to the database and redirect you to your profile. 
       Else, it will return the same page
    '''
    ...
    #if the request was a post request, get the new title and content and add it as a new story
    if method == "POST":
...
def view():
    ...
    #try to get name of story from query 
    try:
        storyname = request.args['story']
    except:
        return redirect("/profile")
    ...
    #checks to see if you can view everything or just the last contribution
    ...
    return render_template('view.html', story = storyname, content = storycontent, prev = getPrevious)
